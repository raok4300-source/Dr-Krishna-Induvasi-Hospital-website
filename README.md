...
Every time someone books an appointment, an email alert is sent automatically
to `raok4300@gmail.com` (set via `NOTIFY_EMAIL`) using a Gmail account as the
sender. This requires a **Gmail App Password** (not your normal Gmail
password):

1. Go to your Google Account → **Security** → turn on **2-Step Verification**
   if it isn't already on (App Passwords require it).
2. Go to **myaccount.google.com/apppasswords**.
3. Choose **Mail** as the app, generate it, and copy the 16-character
   password shown (no spaces).
4. Set these environment variables wherever the server runs (on Render:
   **Settings → Environment**):
   - `GMAIL_USER` = the Gmail address sending the alerts (e.g.
     `raok4300@gmail.com` or a dedicated sending account)
   - `GMAIL_APP_PASSWORD` = the 16-character App Password from step 3
   - `NOTIFY_EMAIL` = who receives the alerts (defaults to
     `raok4300@gmail.com` if not set)

If `GMAIL_USER` / `GMAIL_APP_PASSWORD` aren't set, bookings still save
normally — you just won't get an email. Check the server logs for a warning
if emails aren't going out.
