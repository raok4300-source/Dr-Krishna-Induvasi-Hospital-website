...
// ---------- email notifications ----------
// GMAIL_USER / GMAIL_APP_PASSWORD are the sending account's credentials.
// NOTIFY_EMAIL is who receives the "new booking" alert (defaults to the same account).
const GMAIL_USER = process.env.GMAIL_USER || '';
const GMAIL_APP_PASSWORD = process.env.GMAIL_APP_PASSWORD || '';
const NOTIFY_EMAIL = process.env.NOTIFY_EMAIL || 'raok4300@gmail.com';
...
if (GMAIL_USER && GMAIL_APP_PASSWORD) {
  ...
  console.warn('Email notifications disabled: set GMAIL_USER and GMAIL_APP_PASSWORD to enable them.');
}
...
async function sendBookingNotification(appointment) {
  ...
  const lines = [
    ...
    ...(appointment.guardianName ? [`Guardian's Name: ${appointment.guardianName}`] : []),
    `Phone: ${appointment.phone}`,
    `Email: ${appointment.email || '(not provided)'}`,
    `Patient age: ${appointment.patientAge}`,
    `Preferred doctor: ${appointment.doctor}`,
    `Preferred date: ${appointment.preferredDate}`,
    `Preferred time: ${appointment.timeSlot}`,
    `Reason for visit: ${appointment.message || '(none)'}`,
    `Booking ID: ${appointment.id}`,
    `Received: ${appointment.createdAt}`,
  ];
  try {
    await mailer.sendMail({
      from: `"Hospital Website" <${GMAIL_USER}>`,
      to: NOTIFY_EMAIL,
...
const DOCTORS = new Set([
  'Dr. Krishna Induvasi (Paediatric Surgery / Paediatric Urology)',
...
function validateAppointment(body) {
  ...
  if (name.length < 2) errors.name = 'Please enter the patient name.';
  ...
  return {
    ...
    errors,
    clean: { name, guardianName, phone, email, patientAge, doctor, preferredDate, timeSlot, message },
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/appointments' && req.method === 'POST') {
    try {
      ...
      // Fire-and-forget: don't let a slow/failed email delay or break the booking response.
